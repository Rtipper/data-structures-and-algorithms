'use strict';
const Queue = require('../fifoAnimalShelter.js');
...
describe('QUEUE TESTS', () => {
  ...
  it('Can successfully enqueue multiple values into a queue', () => {
    newQueue.enqueue('Cedar', 'dog').enqueue('Nala', 'cat');
  ...
  it('Can successfully peek into a queue, seeing the expected value', () => {
    expect(newQueue.peek().name).toEqual('Cedar');
  });
  ...
  it('Should return "null" when attempting to add an animal that is not a dog or cat', () => {
    expect(newQueue.enqueue('Bruce', 'shark')).toBeFalsy();
  });
});