...
const isNum = (input) => {
  // Solution code here...
  let inputNew = input.toString();
  let regex = /[0-9]/g;
  return regex.test(inputNew);
};
...
const citiesAtoJ = (arr) => {
  ...
  let regex = /[A-J].\w*/gm;
  let arrayNull = [];
  let regexTwo = /\s([A-J]).\w*/gm;
  arr.forEach(val => {
    if(val.match(regexTwo)){
    } else if (val.match(regex)){
      arrayNull.push(val.match(regex)[0]);
    } else {
    }
  });
  return arrayNull;
...
const matchMonth = (input) => {
  // Solution code here...
  let regex=/(\b[O-o]ctober)/g;
  let regextwo = /([O-o]ct)\b/g;
  let inputNew = input.toString();
  if(inputNew.match(regex)){
    return true;
  }else if(inputNew.match(regextwo)){
    return true;
  }else{
    return false;
  }
